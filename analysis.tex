\section{Analyse des donn\'ees}
Le but de cette manipulation est la caracterisation d'un module optique (OM) at la pr\'eparation du dispositif n\'ecessaire. Cel\`a veut dire:

\begin{center}
\fbox{
\begin{minipage}{0.5\textwidth}
Bas\'e sur les donn\'ees, calculer 
\begin{itemize}
\item le gain $G$ de l'OM,
\item la r\'esolution $\sigma_\mathrm{G}$ de l'OM,
\item le nombre moyen de photoelectrons $\langle n_{\mathrm{pe}}\rangle$ produit par trigger dans l'OM.
\end{itemize}
\end{minipage}
}
\end{center}
\pagebreak
